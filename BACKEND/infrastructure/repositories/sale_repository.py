...
class SaleRepository(SaleRepositoryInterface):
    ...
    def get_today_count(self):
        ...
        Uses the configured local timezone (Asia/Manila) so the count matches
        the date embedded in the receipt number generated by _generate_receipt_number.
        """
        from django.utils.timezone import localdate
        today = localdate()  # local date in TIME_ZONE (Asia/Manila), not UTC
    ...
    def create(self, sale: Sale):
        ...
        for item in sale.items:
            ...
            # Atomically deduct stock from the product (prevents overselling races)
            if item.product_id:
